...
public class TimingProjectCallOutSuccessActivity extends Activity implements View.OnClickListener, CompoundButton.OnCheckedChangeListener, DatePickerDialog.OnDateSetListener, MediaPlayer.OnCompletionListener {
    ...
    private RadioGroup rg_again;

//    public static Activity timingProtectActivity;
...
//        Toast.makeText(this, "请提交数据", Toast.LENGTH_SHORT).show();
    ...
    private void initView() {
        ...
        tv_inviteDate = (TextView) findViewById(R.id.activity_calloutsuccess_new_tv_invitetime);
        rl_inviteDateText = (RelativeLayout) findViewById(R.id.activity_calloutsuccess_new_rl_invitetime);
        ...
        tv_upkeepTime = (TextView) findViewById(R.id.activity_calloutsuccess_new_tv_upkeeptime);
        rl_upkeepTime = (RelativeLayout) findViewById(R.id.activity_calloutsuccess_new_rl_upkeeptime);
        ...
        ll_lostReason = (LinearLayout) findViewById(R.id.activity_calloutsucces_new_ll_lostreason);
        ll_lostReason.setOnClickListener(this);
        timingTypeTextView = (TextView) findViewById(R.id.activity_calloutsuccess_new_tv_timingType);
        timingTypell = (LinearLayout) findViewById(R.id.activity_calloutsuccess_new_ll_timingType);
        timingTypell.setOnClickListener(this);
        inViteView = (LinearLayout) findViewById(R.id.activity_calloutsuccess_new_ll_invitetime);//再次邀约的 LinearLayout activity_calloutsuccess_new_ll_invitetime
        upkeepView = (LinearLayout) findViewById(R.id.activity_calloutsuccess_new_ll_upkeeptime);//已保养日期的 LinearLayout  activity_calloutsuccess_new_ll_upkeeptime
        loastReasomView = (LinearLayout) findViewById(R.id.activity_calloutsucces_new_ll_lostreason);//流失原因的 LinearLayout activity_calloutsucces_new_ll_lostreason
        comeinTimeView = (LinearLayout) findViewById(R.id.activity_calloutsucces_new_ll_comeintime);
        ...
        tv_comeingtime = (TextView) findViewById(R.id.activity_calloutsucces_new_tv_comeintime);
        rl_comeingtime = (RelativeLayout) findViewById(R.id.activity_calloutsucces_new_rl_comeintime);
        tv_lastComeingTime = (TextView) findViewById(R.id.activity_calloutsuccess_new_tv_lastcometime);
        rl_lastComeingTime = (RelativeLayout) findViewById(R.id.activity_calloutsuccess_new_rl_lastcometime);
        ...
        ed_nowdrivermile = (EditText) findViewById(R.id.activity_calloutsucces_new_ed_nowdrivermil);
    }//end initview
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.activity_calloutsuccess_new_save://点击了保存
                if (mediaPlayer != null && mediaPlayer.isPlaying()) {
                    mediaPlayer.pause();
                }
                saveToNet();
                break;
            case R.id.activity_calloutsuccess_new_iv_playIcon://点击了播放
                if (mediaPlayer != null) {
            ...
            case R.id.activity_calloutsuccess_new_rl_invitetime:
            ...
            case R.id.activity_calloutsuccess_new_rl_upkeeptime:
            ...
            case R.id.activity_calloutsucces_new_rl_comeintime://邀约进厂时间
                ...
                break;
            case R.id.activity_calloutsuccess_new_rl_lastcometime://最迟进厂时间
                if (inviteSuccessCheckBox.isChecked()) {
                    if (!datePickerDialog.isAdded()) {
            ...
            case R.id.activity_calloutsuccess_new_ll_timingType:
                Log.d("sdsdasd", "swasas");
                if (inviteSuccessCheckBox.isChecked()) {
                    new MyDialogHelper().showListDialog(this, "请选择", getTimeTypeName(), timingTypeTextView);
                }
                break;
            case R.id.activity_calloutsucces_new_ll_lostreason://流失原因
    ...
    private void parseUploadResult(String result, File f, String id) throws JSONException {
        ...
        if ("Success".equals(status)) {
            ...
            if ("Success".equals(status)) {
                ...
                finish();
//                if (timingProtectActivity instanceof TimingProtectActivity) {
//                    TimingProtectActivity temp = (TimingProtectActivity) timingProtectActivity;
//                    temp.refreshData();
//                    timingProtectActivity = null;
//                }
                Intent i = new Intent("ty.refreshlist.tp");
                sendBroadcast(i);
    ...
    private RequestParams getRequestParams() {
        ...
        if (timingProtect.getPredictDate() == null && timingProtect.getLatelyMaintainDate() == null) {

        } else if (timingProtect.getPredictDate() != null && timingProtect.getLatelyMaintainDate() == null) {
            predictDate = timingProtect.getPredictDate();
        } else if (timingProtect.getPredictDate() == null && timingProtect.getLatelyMaintainDate() != null) {
            predictDate = timingProtect.getLatelyMaintainDate();
        } else if (timingProtect.getPredictDate() != null && timingProtect.getLatelyMaintainDate() != null) {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

            try {
                long time1 = sdf.parse(timingProtect.getPredictDate()).getTime();
                long time2 = sdf.parse(timingProtect.getLatelyMaintainDate()).getTime();
                if (time1 > time2) {
                    predictDate = timingProtect.getLatelyMaintainDate();
                } else {
                    predictDate = timingProtect.getPredictDate();
...
//        if (!validateNotEmpty(ed_nowdrivermile.getText().toString())) {
//            Toast.makeText(this, "请填写目前行驶里程", Toast.LENGTH_SHORT).show();
//            return null;
//        }
        ...
        if (inviteCheckBox.isChecked()) {
            if (!validateNotEmpty(tv_inviteDate.getText().toString())) {
                Toast.makeText(this, "请选再次择邀约日期", Toast.LENGTH_SHORT).show();
                return null;
            }
            //再次邀约
            if (validateDate(tv_inviteDate.getText().toString())) {
                return XutilsRequest.getFirstProjectCallSuccess(frameNum, "true", talkProcessEditText.getText().toString(),
                        ...
                        "0", "",
                        "0", "", "", "", "",
                        user.getEmpId(), "定保", predictDate, ed_nowdrivermile.getText().toString());
            } else {
                Toast.makeText(this, "再次邀约时间不能小于今天", Toast.LENGTH_SHORT).show();
                return null;
            }
            ...
            if (PhoneValidate.compareDataTime(tv_comeingtime.getText().toString(), tv_lastComeingTime.getText().toString())) {
                Toast.makeText(this, "最迟进厂时间不能小于邀约进厂时间", Toast.LENGTH_SHORT).show();
                return null;
            }
    ...
    private void showInfilater(boolean invitetime, boolean upkeeptime, boolean lostreason, boolean comeintime) {
        if (invitetime) {
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        switch (buttonView.getId()) {
            case R.id.activity_calloutsuccess_new_cb_invite://再次邀约
                ...
                break;
            case R.id.activity_calloutsuccess_new_cb_upkeep://已保养
                if (isChecked) {
                    upkeepView.setVisibility(View.VISIBLE);
                    inViteView.setVisibility(View.GONE);
                    loastReasomView.setVisibility(View.GONE);
                ...
                break;
            case R.id.activity_calloutsucces_new_cb_lostcusotmer://流失客户
                if (isChecked) {
                    loastReasomView.setVisibility(View.VISIBLE);
                    inViteView.setVisibility(View.GONE);
                    upkeepView.setVisibility(View.GONE);
                ...
                break;
            case R.id.activity_calloutsuccess_new_cb_invitesuccess://成功邀约
                if (isChecked) {
                    comeinTimeView.setVisibility(View.VISIBLE);
                    inViteView.setVisibility(View.GONE);
                    upkeepView.setVisibility(View.GONE);
                    ...
                    public void onTimeSet(RadialPickerLayout view, int hourOfDay, int minute, int second) {
                        ...
                        if (zuicijingchangdate == null) {
                            ...
                            if (PhoneValidate.compareDataTime(zuicijingchangdate, result)) {
                                ...
                                Toast.makeText(TimingProjectCallOutSuccessActivity.this, "最迟进厂时间不能小于邀约进厂时间", Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
                }, 12, 12, true);
