...
public class XutilsRequest {
    ...
    public static RequestParams getWaiUpdateParams(String name, String phone, String sorce,
        ...
        HashMap<String, String> map = new HashMap<>();
        map.put("CustomerName", name);//客户名字
        map.put("Phone", phone);//客户电话
        map.put("CustomerSource", sorce);//"外展"
        map.put("IntentLevel", intentLevel);//意向等级
        map.put("CarSeriesID", intentCars);//车系ID
        map.put("CarModelID", intentCarModle);//车型ID
        map.put("CarColorID", intentCarColor);//车色ID
        map.put("FocusCarmodelID", feat);//车辆特征  特征ID&特征ID  2&3&4
        map.put("FollowPeo", alongPersonNumber);//随行人数
        map.put("QuoteSituation", "0");
        map.put("ChannelId", storeChannel);//来电渠道ID
        map.put("IsDrive", "0");
        map.put("UseageID", carUse); //购车用途ID
        map.put("PayType", mortgage);//按揭方式
        map.put("TalkProcess", talkPro);//洽谈内容
        map.put("Comedate", receptionDate);//接待时间
        map.put("CampaignName", receptionActv);//外展活动ID
        map.put("ComeTime", receptionStart);//接待开始时间
        map.put("LeaveTime", receptionEnd);//接待结束时间
        map.put("SourceChannelID", infoChannel);//信息渠道ID
        map.put("LiveProvince", province);//省
        map.put("LiveCity", permanent);//市
        map.put("LiveArea", county);//区
        map.put("IsChange", newChange);// 置换“0”  新购“1”
        map.put("NextTime", nextCallTime);//下次回访时间
        map.put("CarNo", turnto);//转介绍人
        map.put("EmpID", empID);
        RequestParams requestParams = new RequestParams();
        String ts = String.valueOf(System.currentTimeMillis());
        String data = getJsonFromMap(map);
...
//        String data = "{\"user\":\"" + username + "\",\"pw\":\"" + sha1Code(password) + "\"}";
        String data = "{\"user\":\"" + username + "\",\"pw\":\"" + password + "\"}";
    ...
    public static RequestParams getVisit(String customerID, String empID, String talkProcess,
                                         ...
                                         String subscribeDate, String remark, String ValidType,String visitType) {
        HashMap<String, String> data = new HashMap<>();
        data.put("CustomerID", customerID);
        data.put("ContactEmpID", empID); //登录人ID
        data.put("TalkProcess", talkProcess);// 谈话记录  必填
        data.put("IntentLevel", intentLevel);//等级  必填
        data.put("IsSuccess", isSuccess); //
        data.put("FailBackType", failBackType);
        data.put("BackDateTime", bakDateTime);//预计回访时间  必填
        data.put("FailType", failType);
        data.put("OtherBrandName", otherBrandName);
        data.put("OtherSeriesName", otherSeriesName);
        data.put("IsSubscribe", isSubscribe);//是否邀约
        data.put("QuoteSituation", quoteSituation);//报价情况 无用
        data.put("SubscribeDate", subscribeDate);
        data.put("Remark", remark);
        data.put("FailAnalysis", "");
        data.put("ValidType", ValidType);//是否有效
        data.put("VisitType", visitType);// 标记   潜客 或者线索
    ...
    public static RequestParams cpPhoneCustomer(String empId,String CustomerPhone, String ContactLogID) {

        HashMap<String, String> map = new HashMap<>();
        map.put("CustomerPhone", CustomerPhone );
        map.put("ContactLogID", ContactLogID );
        map.put("EmpID", empId);
...
//        map.put("reciveComeID", cpCustomer.getReciveComeId());
        ...
        map.put("CarSeriesID", cpCustomer.getCarSeriesID());
        map.put("CarModelID", cpCustomer.getCarModelID());
...
//3.0
        map.put("SparePhone", cpCustomer.getCustomerSparePhone());
        map.put("CusLiveAddress", cpCustomer.getAddressInfo());
        map.put("CusSex", cpCustomer.getCusSex());
        map.put("ChannelRemark", "");
        map.put("CarBrandID", cpCustomer.getCarBrandID());
        map.put("WishBuyDate", cpCustomer.getWishBuyDate());//ID
        map.put("Remark", cpCustomer.getRemark());
        map.put("WeChatNum", cpCustomer.getWeChatNum());
        map.put("CusHandyTimeFrame", cpCustomer.getCusHandyTimeFrame());//id

        map.put("CustomerStateStr", cpCustomer.getCustomerStateStr());//id

        map.put("ComeBeginTime", cpCustomer.getComeTime());
        map.put("ComeEndTime", cpCustomer.getLeaveTime());
        map.put("ReciveComeEmpName", "");
        map.put("NextCallTime", cpCustomer.getNextCallTime());

        map.put("ContactLogID", cpCustomer.getReciveComeId());//id
        map.put("CusOwnerID", cpCustomer.getCusOwnerID());//id
        map.put("LoginEmpID", empId);
