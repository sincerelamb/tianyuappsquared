...
public class PhonStateLisen extends PhoneStateListener {
    ...
    private static final int PHONE_SQ = 2;
    private static int type;
    private static int type_pt_clue;
     ...
     */
    public static void setParamsForPtCustomer(Activity a, PtCustomer customer, String path, File recordFiles, int types, int typs_pt_cl) {
        type_pt_clue = typs_pt_cl;
        activity = a;
        recordFile = recordFiles;
        type = types;
        if (type == PHONE) {
            phoneNumber = customer.getCustomerPhone();
        } else if (type == PHONE_SQ) {
            phoneNumber = customer.getSparePhone();
        } else {
            phoneNumber = customer.getCustomerPhone();
        }
    ...
    private void dealClueCallAfter() {
        ...
        if (number.equals(nowClue.getCluePhone()) && duration != 0) {
            ...
            intent.putExtra("type", type_pt_clue);
...
//            ClueReturnVisitFail.clueActivity = activity;
    ...
    private void delaPtCustomerCallAfter() {
        ...
        String phone_s = "";
        if (type == PHONE) {
            phone_s = nowCostomer.getCustomerPhone();
        } else if (type == PHONE_SQ) {
            phone_s = nowCostomer.getSparePhone();
        } else {
            phone_s = nowCostomer.getCustomerPhone();
        }
        Cursor query = resolver.query(CallLog.Calls.CONTENT_URI,
                null, "number=?", new String[]{phone_s}, CallLog.Calls.DATE + " desc limit 1");
        ...
        if (num.equals(phone_s) && duration != 0) {
            Log.i(LOG_TAG, "[ok]");
            Intent intent = new Intent();
